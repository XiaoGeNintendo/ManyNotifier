...
public class POJSubmission {
	 ...
	 */
	/**
	 * Construct from HTML code
	 * 
	 * @param tr
	 *            Table
	 * @throws Exception
	 *             When parsing error occures
	 */
	public POJSubmission(String tr) throws Exception {
		tr = "<table>" + tr + "</table>";
		Document d = Jsoup.parse(tr);
		Element tds = d.body().getElementsByTag("table").get(0).getElementsByTag("tbody").get(0).getElementsByTag("tr")
				.get(0);
